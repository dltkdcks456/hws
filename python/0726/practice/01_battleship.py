...
# 1. 게임 준비
while True:
    where = int(input('배를 위치시킬 시작점을 고르세요. : ')) # 1-1) 플레이어의 배 시작 위치 고르기
    if 0 <= where <=13: 
        break
    else: # 1-2) 범위를 벗어난 시작점을 고른 경우
        print('\n-----해당 위치에는 배를 둘 수 없습니다.-----\n')


computer_position = (random.sample(range(13), 1))[0]

# 1-3) 컴퓨터의 배 시작 위치를 랜덤으로 지정합니다.

set_ship(where - 1, player_sea)
set_ship(computer_position, computer_sea)
...
플레이어는 컴퓨터의 해역 {player_attack}번째 칸을 공격하였고, 컴퓨터의 배는 피격되었습니다.
게임이 종료되었습니다! {round}라운드 만에 플레이어의 승리입니다!''')
    ...
    computer_unattacked = [position for position in range(len(computer_attacked)) if computer_attacked[position] == False]
    print(computer_unattacked)
    computer_attack = random.sample(computer_unattacked, 1)[0]
    # 2-5) 컴퓨터의 공격 위치 지정
    # 컴퓨터가 공격하지 않은 위치를 나타내는 리스트
    if player_sea[computer_attack] == 1:
        print(f'''<{round}라운드 결과!>
플레이어는 컴퓨터의 해역 {player_attack}번째 칸을 공격하였으나, 공격에 실패하였습니다!
컴퓨터는 플레이어의 해역 {computer_attack + 1}번째 칸을 공격하였고, 플레이어의 배는 피격되었습니다.
게임이 종료되었습니다! {round}라운드 만에 컴퓨터의 승리입니다''')
        break
    # 2-6) 컴퓨터의 공격이 성공한 경우
    else:
        computer_attacked[computer_attack] = True
        print(f'''<{round}라운드 결과!>
플레이어는 컴퓨터의 해역 {player_attack}번째 칸을 공격하였으나, 공격에 실패하였습니다.
컴퓨터는 플레이어의 해역 {computer_attack + 1}번째 칸을 공격하였으나, 공격에 실패하였습니다!''')
        round = round + 1
    # 2-7) 컴퓨터의 공격이 실패한 경우
